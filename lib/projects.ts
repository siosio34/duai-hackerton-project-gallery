...
export const projects: Project[] = [
  {
    ...
    overview:
      "사실 이 프로젝트의 진짜 질문은 '게임을 만들 수 있나'보다 '게임에 들어가는 창작 요소를 AI로 어디까지 만들 수 있나'였다. 개발자라면 한 번쯤 품는 '내 게임' 로망에서 출발해, 두두의 캐릭터 모티브는 신디에게서 얻고 프롤로그 영상은 Seedance 2로 뽑았다. 캐릭터·효과음·UI·레벨 생성까지 제작의 상당 부분을 생성형·프로시저럴로 직접 찍어냈다 (텍스처·HDRI·BGM 등 일부는 CC0 에셋 — 아래 리소스 참고).",
    features: [],
    strengths: [
      {
  ...
  },
  {
    ...
    features: [
      ...
      "하루 동선을 자동으로 추천한다. 가까운 곳끼리 묶되 하루에 걸을 수 있는 도보 한계 시간과 대중교통 이동까지 고려해 무리 없는 Day별 루트로 펼치고, 미식 집중·여유 하이라이트·동네별 3가지 프리셋으로 성격을 바꾼다.",
  ...
  },
  {
    ...
    features: [
      ...
      "종목 딥리서치 — 티커 한 줄로 4축(시세·퀀트 factor·Kronos 20일 ML 예측·LLM 내러티브)을 종합해 '검토 포인트' 브리프를 만든다. 기술적 차트 분석(이평선·지지/저항·RSI·볼린저밴드), 동종 섹터 경쟁 비교, 실적발표·전문가 리포트·관련 영상 요약, 투자 거장 18인 관점, bull/bear 2라운드 토론, bear·base·bull 시나리오 가격, 그리고 Seeking Alpha의 밸류에이션·성장성·수익성·모멘텀 등급과 애널리스트·월가·퀀트 평점까지 한 종목 안에 펼친다. 각 축은 독립 실패를 허용해 CDP 미접속·모델 부재 시 그 축만 mock으로 빠지고 나머지로 완전한 스냅샷을 반환한다.",
      "달러 매수 타이밍 참고 — 원/달러·DXY·달러 갭 비율·적정 환율 4개 지표를 O/X 게이트로 묶어 휴리스틱 분할 환전 '참고 신호'를 낸다. 확정 조언이 아니라 통과 게이트 수로만 말한다.",
      "소셜 트렌드 타임라인 — Reddit·YouTube·X(grok)·ApeWisdom에서 engagement 높은 글과 해외 주식 갤러리에서 자주 언급되는 종목(모멘텀)을 모아 번역·클러스터링해 Today's Social Pulse·Hot Topics·종목 멘션·sentiment와 '왜 화제인지' 3줄을 정리한다.",
    ...
    strengths: [
      ...
      },
      {
        ...
        body: "모든 패널은 LIVE/mock 모드 배지와 소스·업데이트 시각을 달고 산다. 데이터는 공식 API·RSS·유료 구독 세션으로 직접 호출하는데, 로그인해 둔 브라우저에 Chrome CDP로 붙어(집에 띄워 둔 에이전트 PC) 약관을 존중하는 read-only 수집을 주기적으로 돌린다. 시크릿은 로컬 백엔드/프록시에만 둬 클라이언트에 노출하지 않는다.",
      },
    ],
    outcome:
  ...
  },
  {
    ...
    features: [
      ...
      "한국 법률 스크리닝 — 개인정보보호법·전자금융거래법·자본시장법·표시광고법 등에서 심각도(LOW~BLOCKER)·트리거 도메인·자문 질문을 조항 단위로 도출한다. korea-law-mcp로 실제 법령 조문을 끌어와 근거에 붙이며, 이슈 스파팅이지 합법 단정이 아니다.",
