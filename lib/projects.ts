...
export const projects: Project[] = [
  ...
  },
  {
    ...
    overview:
      "주제나 키워드를 받으면 여러 관점으로 깊게 파고들어, 근거가 인용된 긴 HTML 보고서로 내놓는다. 핵심은 '파일이 곧 상태'라는 설계다. 브리프, 증거 원장, 관점별 노트, 종합까지 중간 산출물을 전부 research 디렉터리에 쌓아두기 때문에, 대화가 중간에 잘려도 파일에서 그대로 이어간다. 원문은 서브에이전트 안에서만 읽고, 밖으로 넘기는 건 인용을 담은 원장 항목과 250단어 요약뿐이다. 한국어 보고서라면 마지막에 humanize-korean을 한 번 더 돌려 번역투나 판에 박힌 마무리 같은 AI 티를 걷어낸다.",
    problem:
      "그냥 물어보면 답이 짧고 얕았다. 인용은 부실하고, 과장은 걸러지지 않은 채 그대로 나왔다. 읽었다는 자료와 실제로 인용한 자료가 따로 놀고, 무엇이 검증된 사실인지 가려낼 기준도 없었다. 게다가 스킬이 정말 나은지 정직하게 보여줄 비교 근거조차 없다. 그럴듯해 보이는 보고서와, 따져 물어도 버티는 보고서는 다르다.",
    motivations: [
      {
    ...
    role: "혼자 만들었다. 여러 단계로 나뉜 오케스트레이션 파이프라인(Phase 0~5), 신뢰도 등급을 매기는 증거 원장, 기계 검증기(validate_report.py), 다른 모델로 교차 리뷰하는 council과 라이브 시그널·영상 자막 수집, 한국어 humanize 연동, 그리고 효과를 실제로 잰 A/B 비교 앱까지 전부 설계했다.",
    features: [
    ...
    outcome:
      "같은 주제를 같은 날, 같은 한국어로 두 방식으로 돌려 정직하게 비교했다(엔비디아 젠슨 황 방한). 본문 인용은 10건에서 182건으로 18.2배, 고유 출처는 10건에서 141건으로 14.1배 늘었다. 출처 유형은 2종에서 11종, 1차·공식 출처는 0건에서 21건, 영상 자막 원문은 0건에서 7건, 정량 차트는 0개에서 3개, 적대적 검증은 0라운드에서 4라운드가 됐다. 비용도 숨기지 않았다. 88초가 약 53분으로 36배, 측정된 서브에이전트 토큰은 약 99만이다. 불리한 지표는 빼지 않고 2차 지표로 내려 그대로 적었다. 일반응답이 단정했던 '슈퍼을 격상', '방한이 증시를 끌어올렸다', 'GPU 26만 장 확정계약'은 검증을 거치며 모두 강등됐다. 길어진 게 아니라, 따질 수 있게 된 것이다.",
    links: [
      {
        label: "실제 보고서 보기",
