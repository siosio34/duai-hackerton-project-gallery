...
export const projects: Project[] = [
  ...
  },
  {
    ...
    title: "코인모으기 With 군인",
    tagline:
      "군 장병이 군 증빙을 올려 인증하면, 입대·진급·전역 세 시점마다 실제 코인을 지갑으로 자동 지급하는 마케팅 이벤트 백엔드. 거래소 적립 메커니즘은 건드리지 않고 인증·보상 레이어만 새로 얹었다.",
    section: "projects",
    category: "Backend",
    tags: ["군 인증 보상", "하이브리드 검수", "자동 보너스 계산", "스펙 우선 개발"],
    tech: [
      "Python",
      "pytest",
      "WSGI (wsgiref)",
    ...
    visual: "glyph",
    media: {
      poster: "/media/coin-collect-with-soldier/poster.svg",
      thumbnail: "/media/coin-collect-with-soldier/thumb.svg",
    },
    overview:
      "군 장병을 위한 '코인 모으기' 마케팅 이벤트를 그린필드로 새로 지은 백엔드. 참여자가 군 신분증·병적증명서를 올리면 자동 1차 + 관리자 2차 하이브리드로 증빙을 검수하고, 인증이 끝나면 입대일과 최초 계급을 뽑아 세 갈래로 보상한다 — 인증 즉시 받는 Layer 1 기본, 진급 시점마다 자동 지급되는 Layer 2, 전역 시점에 적립 유지 여부로 차등 지급되는 Layer 3. 모든 보상은 포인트·쿠폰이 아니라 실제 코인으로 참여자 지갑(wallet_address)에 송금되고 tx_hash로 기록된다. 기존 거래소의 적립 메커니즘(출석·매수·미션)은 외부 API로 호출만 하고 본체 코드는 한 줄도 손대지 않는다.",
    problem:
      "이벤트 하나를 위해 거래소 본체에 군인 전용 적립 로직을 박아 넣으면, 검증된 코어가 흔들리고 이벤트가 끝나도 흔적이 남는다. 게다가 군 증빙 검수는 외부 OCR·신분증 검증 API에 맡기기엔 민감하고, 진급·전역 보너스를 받겠다고 장병에게 매번 재증빙을 시키는 것도 가혹하다. 필요한 건 거래소 바깥에 독립적으로 서서, 증빙은 자체적으로 검수하고, 최초 인증 한 번으로 진급·전역까지 알아서 계산해 실제 코인을 쏘는 인증·보상 레이어였다.",
    role: "혼자 만들었다. 증빙 업로드·검수, 군 계급 진급 규칙, 3단계 코인 지급, 거래소 적립 이벤트 연동, 운영 튜닝용 EventConfig까지 도메인 전체를 설계했다. 코드를 짜기 전에 Ouroboros로 요구사항을 못 박아(seed.yaml) 숨은 가정을 먼저 드러낸 뒤, 책임이 하나씩만 있는 작은 모듈로 쪼개 단위·통합 테스트로 덮었다.",
    diagram: {
      image: "/media/coin-collect-with-soldier/flow.svg",
      caption:
        "증빙 한 장이 실제 코인 지급으로 이어지기까지. 업로드 → 자동 1차 검수 → 임계 판정(승인·보류·반려, 임계 미구성 시 fail-safe로 전건 관리자 대기) → 군 인증 완료 → 3단계 보상. 아래 레인은 거래소 적립 이벤트를 외부 API로만 받아 Layer 3 유지 판정에 쓰는 경로다.",
    },
    strengths: [
      {
        title: "코드보다 스펙을 먼저 못 박았다",
        body: "Ouroboros 스펙-우선 워크플로로 요구사항을 seed.yaml에 고정하고(모호도 0.17까지 수렴) 숨은 가정을 코드 전에 드러냈다. 그 결과 200개 넘는 모듈이 저마다 책임 하나씩만 지도록 잘게 쪼개져, 검수·계급·지급·적립 연동이 서로 독립적으로 교체·테스트된다.",
      },
      {
        title: "fail-safe를 기본값으로 둔 하이브리드 검수",
        body: "자동 1차 검수는 외부 OCR·신분증 API 없이 필드 가중치 점수로 자체 구현했다. 임계는 event_config가 단일 권위 출처이고, 자동 승인·반려 임계가 비어 있으면 자동 판정을 멈추고 모든 증빙을 관리자 검수 대기로 돌린다 — 설정 미비 상태에서 함부로 승인·반려하지 않는다.",
      },
      {
        title: "입대일·계급만으로 굴러가는 자동 보상",
        body: "진급(Layer 2)과 전역(Layer 3)은 재증빙 없이 최초 인증 때 받은 입대일·최초 계급만으로 시점을 계산한다. 한국 육군 병 계급(이병→일병→상병→병장) 진급 규칙으로 promotion_schedule을 만들고, 각 항목을 promotion_schedule_id로 보너스 지급과 1:1로 묶는다.",
      },
      {
        title: "거래소는 건드리지 않는 연동 경계",
        body: "거래소 적립 메커니즘은 외부 API로 호출만 하고 본체 코드는 수정하지 않는다. 적립 이벤트는 웹훅으로 받아 event_id 멱등 dedup으로 중복을 거르고, 중복 지급 가드까지 둬 같은 보상이 두 번 나가지 않게 했다.",
      },
    ],
    features: [
      "3단계 실제 코인 보상 — Layer 1은 인증 완료 즉시 기본 보너스, Layer 2는 진급 시점마다 추가, Layer 3는 추정 전역 시점에 적립 유지 판정 결과로 차등 지급한다. 전부 포인트가 아니라 실제 코인을 wallet_address로 송금하고 tx_hash·지급 시각을 남긴다.",
      "자체 구현한 자동 1차 검수 — 업로드된 증빙의 필드를 가중치로 채점해 점수를 내고, 임계에 따라 승인·반려·관리자검수필요로 가른다. 외부 OCR·신분증 검증 API를 쓰지 않고, 회색지대 점수는 관리자 2차 검수 화면으로 넘긴다.",
      "입대일·최초 계급 기반 진급 자동 계산 — 한국 육군 병 계급 법정 진급 주기로 추정 진급 시점 목록(promotion_schedule)을 만들고, 진급 시점마다 Layer 2 보너스를 자동으로 지급한다. 진급 주기는 event_config로 오버라이드된다.",
      "Layer 3 전역 유지 판정 — 인증 완료부터 추정 전역까지 수신된 거래소 적립 이벤트 중 해당 participant_id 귀속 건수를 월별 누적 집계(collection_retention_metric)하고, layer3_retention_threshold와 비교해 success/fail을 판정한다. success 지급액은 Layer 1 기본보다 크다.",
      "코드 수정 없이 튜닝하는 EventConfig — 자동 검수 임계·계급별 진급 주기·군종별 복무기간·Layer 3 유지 임계·레이어별 보상 금액을 전부 설정값으로 노출한다. layer3_success_amount > layer1_base_amount 불변식은 설정 검증 단계에서 강제한다.",
      "외부 의존성 없는 WSGI API + 멱등 처리 — Flask·FastAPI 없이 표준 라이브러리 wsgiref로 관리자 검수 API와 거래소 적립 웹훅을 올렸다. 적립 이벤트는 event_id로 dedup하고, 지급은 idempotency key로 조회·가드해 재시도에도 중복 지급이 없다.",
    ],
    outcome:
      "증빙 업로드 → 자동/관리자 검수 → 군 인증 완료 → Layer 1·2·3 코인 지급으로 이어지는 전 경로를 그린필드로 구현하고, 책임 단위로 쪼갠 200여 개 모듈을 단위·통합 테스트로 덮었다. 검수 임계·진급 주기·복무기간·Layer 3 임계·보상 금액은 코드를 고치지 않고 event_config만으로 바꿀 수 있다. 다만 스펙에서 명시적으로 미룬 범위는 그대로 비워 뒀다 — KPI 목표치·기간·대시보드, 외부 OCR/신분증 검증 연동, 조기전역·의병제대 같은 이례 상황 자동 처리는 다음 단계다. 아직 공개 배포 전이라 라이브 링크 대신 설계와 흐름으로 보여준다.",
    links: [],
    gallery: [],
  },
  {
  ...
  },
  {
    ...
    tagline:
      "다 만든 결과물을 평가자 여러 명에게 따로 채점받고, 기준을 넘을 때까지 알아서 고치고 다시 채점하는 Claude Code 스킬.",
    section: "skills",
    category: "Skill",
    ...
    overview:
      "기획서, 보고서, 사업제안서, 카피, 코드처럼 다 만든 결과물을 놓고 평가자 여러 명이 각자 점수를 매긴다. 기준에 못 미치면 스스로 고친 뒤 다시 채점하고, 기준을 넘거나 정해 둔 횟수에 닿을 때까지 이 과정을 반복한다. 같은 채점 엔진을 두 가지로 쓸 수 있다. 결과물만 있으면 그 결과물을 직접 손봐서 올리고(artifact 모드), 평가 대상이 스킬이면 스킬을 여러 입력에 돌려 나온 결과를 채점한 다음 결과물이 아니라 스킬 문서(SKILL.md)를 고쳐 다음 실행이 통과하게 만든다(skill 모드).",
    problem:
      "한 모델한테만 채점을 맡기면 점수가 서로 비슷하게 쏠리고, 자기가 만든 걸 후하게 보는 편향이 끼기 쉽다. '괜찮아 보인다'와 '기준을 넘었다'는 다른 얘기다. 게다가 흔한 품질 개선 루프는 결과물 하나만 고치고 끝나서 정작 그 결과물을 만들어 내는 스킬은 그대로 남는다. 그러면 같은 약점이 다음 결과물에서도 똑같이 반복된다. 채점을 객관적으로 거르는 장치도, 결과물이 아니라 그걸 만드는 쪽을 고치는 장치도 그동안 없었다.",
    role: "혼자 만들었다. 페르소나를 뽑아 오는 부분, 여러 모델을 묶어 채점하는 부분, 점수를 모아 통과 여부를 가르는 부분, 그리고 결과물과 스킬을 각각 자동으로 고치는 두 루프까지 평가 과정 전체를 직접 설계했다.",
    features: [
      "채점은 성격이 다른 세 평가자가 따로 한다. Claude 서브에이전트(native), 컴퓨터에 깔린 Codex와 Grok(external, API 키가 필요 없다), 한국 사용자 페르소나 패널(persona, 대상이 한국이면 자동으로 붙는다). 서로 독립적이라 한 모델의 편향이 점수 전체를 끌고 가지 못한다.",
      "통과 여부는 사람 느낌이 아니라 정해진 규칙으로 가른다. 종합 점수가 기준선(기본 8.0)을 넘고, 동시에 세 평가자 점수가 모두 바닥선(기본 6.0)을 넘어야 통과다. 한 평가자라도 바닥이면 통과시키지 않아서, 평균만 좋고 한쪽이 무너진 결과물을 걸러낸다.",
      "artifact 모드: 기준에 못 미치면 개선 담당이 '꼭 고쳐야 할 것'만 반영해 결과물 파일을 직접 고치고 다시 채점한다. 통과하거나 정해 둔 횟수에 닿을 때까지 반복한다.",
      "skill 모드: 스킬을 여러 입력에 돌려 나온 결과를 채점하되, 실제로 손대는 건 스킬 문서다. 한 입력에만 맞춰 통과하는 걸 막으려고 가장 낮은 점수를 기준으로 삼는다. 그래야 스킬이 두루 통하게 다듬어진다.",
      "한국 페르소나는 nvidia/Nemotron-Personas-Korea(약 100만 명, 26개 항목)에서 대상에 맞게 뽑아 쓴다. 관련도에 따라 1~3점 가중치를 두고 10점으로 종합한 뒤 GO·조건부 GO·PIVOT·KILL로 판정하고, 페르소나마다 그렇게 본 이유까지 풀어서 보여준다.",
      "라운드마다 세 평가자의 점수와 페르소나별 코멘트, 직전 대비 변화량을 리포트로 남긴다. 채점은 병렬로 돌리고, 개선 담당은 한 모델이 막히면 다른 모델로 넘어가게 해서 루프가 멈추지 않는다. 기준을 넘긴 뒤에도 점수를 더 올려 보고, 마지막엔 그중 가장 잘 나온 버전을 결과물로 남긴다.",
    ],
    outcome:
      "실제로 'AI 자동매매 봇 기획서' 초안 하나를 artifact 모드로 돌리면서 라운드마다 바뀌는 화면을 그대로 찍어 아래에 붙였다. 같은 초안의 종합 점수는 4.0에서 5.8, 6.0, 6.3으로 올라갔다. 그런데 평가자별로 나눠 보면 얘기가 갈린다. 글의 완성도를 보는 native는 4.0에서 6.7로 또렷하게 오르는데, 한국 사용자 신뢰를 보는 persona는 2.7에서 3.6에 머물러 바닥선 6을 끝내 못 넘었다. 설명과 단서, KPI 표가 붙으면서 문서는 꽤 그럴듯해 보이지만, 정작 일반 사용자 눈에는 리스크가 더 도드라져 신뢰가 따라오지 않은 것이다. 점수 하나로만 봤으면 6.3 평균에 묻혔을 차이인데, 규칙이 persona 바닥선 미달을 잡아내 통과시키지 않고 멈췄다. 끝에는 '백테스트 기준을 데이터셋·기간·슬리피지·OOS·중단 조건까지 내려서 잡아라'처럼 문장만 손봐서는 풀리지 않는 요구가 계속 남는 게 감지돼, 점수를 억지로 올리는 대신 사람이 볼 몫으로 넘기고 가장 점수가 좋았던 버전을 남겼다. 어디까지가 '잘 쓴 글'이고 어디부터가 '믿을 만한 계획'인지를 숫자와 코멘트로 갈라 보여주는 게 이 스킬이 하는 일이다. (이번 실행에선 Grok이 크레딧이 떨어져 external은 Codex 하나로만 채점했다. 합성 페르소나와 모델 채점은 방향을 가늠하는 신호일 뿐, 실제 시장 조사를 대신하지는 않는다.)",
    showcases: [
      {
        title: "라운드를 거치며 점수가 오르는 화면",
        early: true,
        cols: 4,
        aspect: "wide",
        caption:
          "같은 'AI 자동매매 봇 기획서' 초안 하나를 artifact 모드로 돌리면서 단계별 화면을 라운드마다 찍었다. 위쪽 막대그래프와 왼쪽 목록의 점수가 라운드를 지나며 올라가는 걸 볼 수 있다. 종합은 4.0에서 6.3까지 오르지만, 글의 완성도(native)가 4.0에서 6.7로 오르는 동안 한국 사용자 신뢰(persona)는 2.7에서 3.6에 머물러 바닥선 6을 못 넘는다. 그래서 규칙은 평균에 속지 않고 persona 미달로 멈춘다. 마지막 화면 위쪽의 '에스컬레이션' 배너는, 문장만 손봐서는 못 고치는 설계 문제(백테스트 기준을 데이터셋·기간·슬리피지·OOS·중단 조건까지 내리는 일)를 사람에게 넘겼다는 표시다. 썸네일을 누르면 크게 볼 수 있다.",
        items: [
          {
            src: "/media/eval-improve-skills/rounds/round1.png",
            label: "Round 1 · 종합 4.0 · native 4.0 / external 4.0 / persona 2.7",
          },
          {
            src: "/media/eval-improve-skills/rounds/round2.png",
            label: "Round 2 · 종합 5.8 · native 5.7 / external 6.0 / persona 3.4",
          },
          {
            src: "/media/eval-improve-skills/rounds/round3.png",
            label: "Round 3 · 종합 6.0 · native 6.0 / external 6.0 / persona 3.4",
          },
          {
            src: "/media/eval-improve-skills/rounds/round4.png",
            label: "Round 4 · 종합 6.3 · native 6.7 / external 6.0 / persona 3.6 · 여기서 멈춤",
          },
        ],
      },
    ],
