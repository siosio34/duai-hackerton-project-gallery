...
const DIFFS: Diff[] = [
  {
    ...
    exp: "26만 장은 2025-10 APEC 경주의 블랙웰 이니셔티브이고, 2026-06 방한은 HBM4 퀄·SK 파트너십·네이버 기가와트급 AI팩토리 계획 — 두 사건을 분리. 26만 장 중 구속력 있는 계약은 정부 1.46조·13,136장뿐임을 명시(반증 검증 V1 + council 교차).",
  },
  {
  ...
  return (
    <article className="mx-auto max-w-[1180px] px-5 pb-24 pt-10 sm:px-8">
      ...
      <section className="mt-24">
        ...
        <Reveal delay={0.05}>
          <p className="mt-5 max-w-[78ch] break-keep text-sm leading-relaxed text-ink-mute">
            ...
            가장 직관적인 비교는 생성 토큰 약 16.7배. 비용은 Opus 4.8 종량제 단가 환산치이며 Max/Pro
            구독 청구액이 아니다 — 총 처리 4,486만 중 4,350만(cache_read)은 1/10 단가라 ‘4천만’은
            처리량이지 그만큼 비싼 게 아니다.
          </p>
        </Reveal>
      </section>
      ...
      <section className="mt-20 space-y-4 border-t border-line pt-8 text-sm leading-relaxed text-ink-mute">
        <p>
          <span className="font-medium text-ink-soft">면책</span> — 본 비교 페이지와 양쪽 산출물은 정보
          제공·방법론 비교 목적이며 투자 자문이나 특정 종목의 매매 권유가 아니다. 모든 종목·수치는 기재된
          출처·날짜 기준이고 추정은 추정으로 표기했다.
        </p>
        <p>
          <span className="font-medium text-ink-soft">측정 방법</span> — 토큰은 세션 트랜스크립트(JSONL)의
          assistant usage를 두 트랙의 타임스탬프 구간으로 필터·합산한 실측값이다. 글자수는 validator,
          시간은 wall-clock, 웹검색은 호출 카운트. council 외부 CLI(grok/codex) 토큰만 미포함이다. 단일
          출처:{" "}
          <a
